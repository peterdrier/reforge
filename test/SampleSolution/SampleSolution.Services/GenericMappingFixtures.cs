...
/// ordinal comparison in <c>MisplacedAnalyzer.SameParameterType</c> necessary: the two type
/// parameters are different symbols, so comparing them by identity called a decisive collision a
/// near-miss.
/// </summary>
...
/// <summary>
/// A DTO by <b>config rule only</b> — the name matches the default <c>*Result</c> pattern, while
/// the <c>Describe</c> method disqualifies it from the structural <c>IsDataCarrier</c> test.
/// </summary>
...
/// <c>ref</c> vs <c>out</c> vs <c>in</c>, so a caller's <c>TryPassthrough(ref string)</c> cannot
/// join this type — comparing the enum exactly called that a near-miss.
/// </summary>
...
/// <summary>A configured-only DTO whose useful properties are <b>inherited</b>.</summary>
...
/// <summary>
/// Constructed rather than called. Three of these in a method are three touches on this section,
/// and counting only member accesses missed them entirely — <c>new T()</c> carries its constructor
/// on the creation expression, while the type name inside it binds to the type.
/// </summary>
...
/// <summary>
/// An enum in another section, for the data/behavior split. Reading its members is reading
/// constants — counted as calls they both mis-describe the evidence and let a type that cannot
/// declare a method win the destination contest.
/// </summary>
...
/// <summary>
/// Reached through an indexer. The indexer symbol hangs off the element-access expression, not off
/// any identifier, so reads through it were measured as nothing.
/// </summary>
...
/// <summary>
/// A base declaring two overloads that differ only in how the parameter is passed, one of which
/// supplies a derived type's interface member.
/// </summary>
...
/// <summary>
/// A contract supplied to a <b>private</b> derived type. The classifier drops effectively private
/// types, so an index built from that list never saw this pin — while the compiler does not care
/// who can see the implementer: moving the base method still breaks the build.
/// </summary>
...
/// <summary>
/// Same-named nested types in one namespace. A destination name assembled from namespace and simple
/// name alone renders both identically, which is what the qualified-name fix had to keep apart.
/// </summary>
...
/// <summary>
/// An indexer declared on an unconfigured, behaviorful base. Read through a configured DTO those
/// reads are data; resolved from the declaring base — which no rule names — they counted as
/// behavior calls.
/// </summary>
...
/// <summary>Behavior reached through <b>operators</b> rather than through named members.</summary>
...
/// <summary>A configured-only DTO whose properties are inherited and read through a property pattern.</summary>
